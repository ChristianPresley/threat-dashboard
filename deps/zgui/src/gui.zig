...
pub const io = struct {
    ...
    /// True when ImGui wants its settings persisted (layout changed etc.).
    /// Pair with `clearWantSaveIniSettings` after a manual save.
    pub const getWantSaveIniSettings = zguiIoGetWantSaveIniSettings;
    extern fn zguiIoGetWantSaveIniSettings() bool;
    pub const clearWantSaveIniSettings = zguiIoClearWantSaveIniSettings;
    extern fn zguiIoClearWantSaveIniSettings() void;
...
/// Serialize ImGui's settings (dock layout, window rects, table state) to an
/// internally-owned buffer — valid until the next save or context teardown.
/// Enables crash-safe manual persistence (write tmp + atomic rename) instead
/// of ImGui's in-place ini writes.
pub fn saveIniSettingsToMemory() []const u8 {
    var size: usize = 0;
    const ptr = zguiSaveIniSettingsToMemory(&size);
    return ptr[0..size];
}
extern fn zguiSaveIniSettingsToMemory(out_size: *usize) [*]const u8;

pub fn loadIniSettingsFromMemory(data: []const u8) void {
    zguiLoadIniSettingsFromMemory(data.ptr, data.len);
}
extern fn zguiLoadIniSettingsFromMemory(data: [*]const u8, size: usize) void;
...
pub const Cursor = enum(c_int) {
    // Matches ImGuiMouseCursor_ in the vendored ImGui 1.92 (imgui.h) —
    // 1.91.7 inserted wait/progress before not_allowed.
    none = -1,
    arrow = 0,
    text_input,
    resize_all,
