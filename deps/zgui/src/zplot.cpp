...
extern "C"
{
    ...
    ZGUI_API bool zguiPlot_DragLineX(
        int id,
        double *x,
        float col[4],
        float thickness,
    ...
    ZGUI_API bool zguiPlot_DragLineY(
        int id,
        double *y,
        float col[4],
        float thickness,
    ...
    ZGUI_API void zguiPlot_Annotation(
        double x, double y,
        float col[4],
        ...
        bool round)
    {
        ...
        ImPlot::Annotation(x, y, (*(const ImVec4 *)&(col[0])), p, clamp, round);
    }

    ZGUI_API void zguiPlot_AnnotationText(
        double x, double y,
        float col[4],
        const float pix_offset[2],
        bool clamp,
        const char *fmt, ...)
    {
        ...
        va_list args;
        va_start(args, fmt);
        ImPlot::AnnotationV(x, y, (*(const ImVec4 *)&(col[0])), p, clamp, fmt, args);
        va_end(args);
    }
    ...
    ZGUI_API void zguiPlot_PlotHeatmapF64(
        const char *label_id,
        const double *values,
        int rows,
        int cols,
        ...
        const double bounds_min[2],
        const double bounds_max[2],
        ImPlotHeatmapFlags flags)
    {
        ImPlot::PlotHeatmap(
            ...
            scale_max,
            label_fmt,
            ImPlotPoint(bounds_min[0], bounds_min[1]),
            ImPlotPoint(bounds_max[0], bounds_max[1]),
            flags);
    }

    ZGUI_API double zguiPlot_PlotHistogramF64(
        const char *label_id,
        const double *values,
        int count,
        int bins,
        ...
        ImPlotHistogramFlags flags)
    {
        return ImPlot::PlotHistogram(
            ...
            bins,
            bar_scale,
            ImPlotRange(range_min, range_max),
            flags);
    }

    ZGUI_API void zguiPlot_ColormapScale(
        const char *label,
        double scale_min,
        double scale_max,
        const float size[2],
        ...
        ImPlotColormap cmap)
    {
        ImPlot::ColormapScale(label, scale_min, scale_max, ImVec2(size[0], size[1]), format, flags, cmap);
    }

    ZGUI_API void zguiPlot_PushColormap(ImPlotColormap cmap)
    {
        ImPlot::PushColormap(cmap);
    }

    ZGUI_API void zguiPlot_PopColormap(int count)
    {
